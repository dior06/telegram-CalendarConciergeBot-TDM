...
def test_authenticate_with_existing_token(mock_load_credentials, mock_exists, mock_creds, mock_build):
...
def test_authenticate_with_new_token(mock_get_new_credentials, mock_creds, mock_build):
    mock_creds_instance = Mock(valid=True)
...
@patch('google_calendar_integration.Credentials.from_authorized_user_file')
def test_load_credentials_success(mock_from_file):
...
def test_load_credentials_error(mock_from_file):
    google_calendar = google_calendar_integration.GoogleCalendarIntegration()
    creds = google_calendar.load_credentials()
